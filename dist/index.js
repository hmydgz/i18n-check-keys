(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('fs'), require('path'), require('@babel/parser'), require('@babel/traverse'), require('chalk')) :
    typeof define === 'function' && define.amd ? define(['exports', 'fs', 'path', '@babel/parser', '@babel/traverse', 'chalk'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.umd = {}, global.fs, global.path, global.parser, global.traverse, global.chalk));
})(this, (function (exports, fs, path, parser, traverse, chalk) { 'use strict';
    ...
    function completionSuffix(filePath, fileType = 'js') {
        // 判断是否有后缀
        if (/\.\w+$/.test(filePath))
            return filePath;
    ...
    function getImportVariableFileAst(importFilePath) {
        ...
        switch (importFilePath.match(/\.(\w+)$/)[1]) {
            case 'json':
                res = loadJsonAst(importFilePath);
                break;
            default:
                res = loadJsAst(importFilePath);
                break;
        }
    ...
    function getVariableAstPath(_astPath) {
        ...
        const fn = (__astPath) => {
            // @ts-ignore
            if (__astPath.parentPath.node.type === 'Program')
                return;
            // @ts-ignore
            if (__astPath.node.type === 'ObjectProperty')
                _path.unshift(__astPath.node.key.name);
            // @ts-ignore
    ...
    function getLocalePath(_currentDirPath = process.cwd(), _localePath = /locale/) {
        ...
        const _getLocalePath = (_path) => {
            fs.readdirSync(_path, { withFileTypes: true }).forEach((dirent) => {
                ...
                if (/node_modules|git/g.test(filePath))
                    return;
        ...
        localePathList.forEach(v => {
            ...
            try {
                fs.readdirSync(v, { withFileTypes: true }).forEach((dirent) => {
                    ...
                    if (dirent.isDirectory())
                        return;
                    if (!_fileType.test(dirent.name) && dirent.name)
                        return;
                    ...
                    if (dirent.name.includes(benchmarkLang)) {
                        ...
                        // 匹配要检查的语言
                        if (languages.length && !languages.some(v => dirent.name.includes(v)))
                            return;
    ...
    function checkI18nKeys(options = {}) {
        const { localePath = /locale/, benchmarkLang = 'en', languages = [], fileType = 'js', needStopRun = false, } = options;
        return {
